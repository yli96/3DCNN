...
def build_cnn(dimension = '3d', activation = 'softmax', heatmap = False, w_path = None, compile_model = True):
    ...
    def build_conv_3d():
        ...
        model.add(Conv3D(16, kernel_size=(3, 3, 3), input_shape=(train_data.shape[1:]), border_mode='same'))
...
def fit_model(model, v, train_data, train_labels, val_data, val_labels):
    model_weights_file = '3dimg_classifier_weights_%s.h5' %v
    epoch_weights_file = '3dimg_classifier_weights_%s_{epoch:02d}_{val_acc:.2f}.hdf5' %v
    model_file = '3dimg_classifier_model_%s.h5' %v
    history_file = '3dimg_classifier_history_%s.json' %v
...
evaluate_model(model, '3dimg_classifier_model_v1_3d.h5', test_data, test_labels)
